...
#### **By Study**
False Negative Rates were also computed indiviually by study source. The table below displays the false negative rates and the measurements within an order of magnittude of the modeled value (displayed as %).
```{r eval = TRUE, echo = FALSE, include = FALSE}
...
sample_size <- c(nrow(sub_rvm2015), nrow(sub_rvm2016), nrow(sub_rvm2017),
                 nrow(sub_hr2008), nrow(sub_metabolites), nrow(sub_dehy),
                 nrow(sub_bio), nrow(sub_dermal))
fnr <- rbind(rate_rvm2015, rate_rvm2016, rate_rvm2017, rate_hr2008, rate_metabolites,
             rate_dehy, rate_bio, rate_dermal)
fnr <- round(fnr*100, 2)
fnr <- cbind(sample_size, fnr)
fnr <- as.data.frame(fnr)
names(fnr) <- c("sample_size", "fnr", "fnr_10", "fnr", "fnr_10")
rownames(fnr) <- c("Van Meter 2014/15", "Van Meter 2016", "Van Meter 2018", "Henson-Ramsey 2008", 
