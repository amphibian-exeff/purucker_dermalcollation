...
##################
# import data 
combined_data <- vm2015_subset3 #temporary
...
n <- length(combined_data$TissueConc)

#bird model with perez body weight
app_rate <- combined_data$app_rate_g_cm2
conv_rate <- 1000000.0
sa_bird_perez <- 65.15 * (combined_data$bodyweight ^ 0.559)
sa_bird_frac <- 0.5
dermal_af <- 1.0
hours_one <- 1.0
combined_data$tim_direct_avian_1hr_perez <- (app_rate * conv_rate * sa_bird_perez * sa_bird_frac * 
                           dermal_af * hours_one)/combined_data$bodyweight
combined_data$tim_direct_avian_1hr_perez_ratios <- combined_data$tim_direct_avian_1hr_perez / combined_data$TissueConc
min(combined_data$tim_direct_avian_1hr_perez_ratios)
max(combined_data$tim_direct_avian_1hr_perez_ratios)
sum(combined_data$TissueConc > combined_data$tim_direct_avian_1hr_perez)
sum(combined_data$TissueConc*10 > combined_data$tim_direct_avian_1hr_perez)
plot(combined_data$TissueConc, combined_data$tim_direct_avian_1hr_perez)

#bird model with tim body weight
sa_bird_tim <- 10 * (combined_data$bodyweight ^ 0.667)
combined_data$tim_direct_avian_1hr <- (app_rate * conv_rate * sa_bird_tim * sa_bird_frac * 
                           dermal_af * hours_one)/combined_data$bodyweight
combined_data$tim_direct_avian_1hr_ratios <- combined_data$tim_direct_avian_1hr/combined_data$TissueConc
min(combined_data$tim_direct_avian_1hr_ratios)
max(combined_data$tim_direct_avian_1hr_ratios)
sum(combined_data$TissueConc > combined_data$tim_direct_avian_1hr)
sum(combined_data$TissueConc*10 > combined_data$tim_direct_avian_1hr)
plot(combined_data$TissueConc, combined_data$tim_direct_avian_1hr)
df.tim_avian_1hr <- data.frame(x=combined_data$TissueConc, y=combined_data$tim_direct_avian_1hr)

#amphib model with hutchinson body weight
sa_amphib_hutchinson <- 1.131 * (combined_data$bodyweight ^ 0.579)
sa_amphib_frac <- 2.0
...
sum(combined_data$TissueConc > combined_data$tim_direct_amphib_1hr)
sum(combined_data$TissueConc*10 > combined_data$tim_direct_amphib_1hr)
plot(combined_data$TissueConc, combined_data$tim_direct_amphib_1hr)
df.tim_amphib_1hr <- data.frame(x=combined_data$TissueConc, y=combined_data$tim_direct_amphib_1hr)
