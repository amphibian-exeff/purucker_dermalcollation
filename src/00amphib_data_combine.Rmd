...
### Glinski et al. 2018c (Biomarkers)
~~~ Emma - still need to alter "pesticide" and "group" columns to be the standardized "chemical" column (talk to Tom) ~~~~~
...
Four columns were dropped from the original biomarkers data set and the names of two columns were standardized.   
```{r eval = TRUE, echo = TRUE, include = TRUE}
## clean the data
# drop cols
drop_cols <- c("met", "tdt", "bif", "rate")
...
The application rates and soil concentrations were not included in the original biomarkers data set. Both are included in the data set with the following dimensions, column names, and summary:
```{r eval = TRUE, echo = FALSE, include = FALSE}
...
A one-to-one merge was employed for each unique sample ID based on the measured pesticide between the original biomarkers data set and the data set containing the application rates and soil concentrations. Vectors containing the application rates and soil concentrations were joined with the original data set.
```{r eval = TRUE, echo = TRUE, include = TRUE}
# bif extraction
...
dag_biomarker_subset2 <- dag_biomarker_subset[order(dag_biomarker_subset[, 4]),]
...
New columns were created for standarization with the collated data set, the columns were ordered alphabetically, and a local copy was stored. 
```{r eval = TRUE, echo = TRUE, include = TRUE}
# create new columns
...
dag_biomarker_merge <- dag_biomarker_subset4[ ,order(names(dag_biomarker_subset4))]
...
~~~ Emma - still need to merge this data set after determine what to do about chemical column. Will need to also update variable names for VM2018 and Dermal Routes merges.
```{r eval = TRUE, echo = FALSE, include = TRUE}
#pending
```
...
~~~ Emma - still need to update the application rate column ~~~~~~  
