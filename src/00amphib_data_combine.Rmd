...
dag2016_dehy2 <- cbind(dag2016_dehy1, formulation, soil_type, application, 
                       app_rate_g_cm2, exp_duration, soil_conc_ugg)
...
#drop decay products that were not sprayed, keeping only parents
rows_to_drop <- which(dag2016_dehy3$matrix == 'soil')
dag2016_dehy4 <- dag2016_dehy3[-rows_to_drop,]
# parent, time and matrix columns delete
drop_cols <- c("parent","time","matrix")
dag2016_dehy5 <- dag2016_dehy4[,!(names(dag2016_dehy4) %in% drop_cols)]
#View(dag2016_dehy4)
unique(dag2016_dehy5$chemical)
update_atrazine <- which(dag2016_dehy5$chemical == 'atrazine')
dag2016_dehy5$app_rate_g_cm2[update_atrazine] <- 0.00002395 # atrazine g/cm2
update_chloro <- which(dag2016_dehy5$chemical == 'chloro')
dag2016_dehy5$app_rate_g_cm2[update_chloro] <-  0.0000443 # chloro g/cm2
update_metol <- which(dag2016_dehy5$chemical == 'metol')
dag2016_dehy5$app_rate_g_cm2[update_metol] <-  0.00003101 # metol g/cm2
update_tdn <- which(dag2016_dehy5$chemical == 'tdn')
dag2016_dehy5$app_rate_g_cm2[update_tdn] <- 0.00000291 # tdn g/cm2
update_imid <- which(dag2016_dehy5$chemical == 'imid')
dag2016_dehy5$app_rate_g_cm2[update_imid] <- 0.00000539 # imid g/cm2
dim(dag2016_dehy5)
#View(dag2016_dehy5)
#temporarily set chloro parent to 100 pending summing daughter metabolites
dag2016_dehy5$tissue_conc_ugg[update_chloro] <-  100.0

# reorder columns alphabetically to help with merge
#reorder dag2016_dehy5
colnames(dag2016_dehy5)
dag2016_dehy_merge <- dag2016_dehy5[,order(names(dag2016_dehy5))]
